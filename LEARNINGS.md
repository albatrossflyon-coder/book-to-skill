...
- 2026-08-12 — **Fixed and verified (commit `eacf09e`).** `trafilatura` wired in as the primary path, same fallback chain. Re-fetched the same 61 real pages, real count dropped 183→69 (site content shifted slightly since 08-08, same bug class). The actual footer (ad + copyright/trademark) is now fully gone (0x); the remaining 69 are one line — the site's own page masthead/tagline, appearing once per page. It sits where a real article lead-in would (top of the content, styled like a subtitle), so trafilatura reasonably keeps it — this is a genuine, if imperfect, fix, not a total elimination, and that distinction matters when reporting the before/after honestly.

## general

- 2026-08-12 — **`uv add <pkg>` defaults new dependencies into the hard `[project] dependencies` list, not `[project.optional-dependencies]`**, even in a project that deliberately keeps its base install dependency-free (this project's whole design: bs4/pypdf/etc. are all optional extras, format-by-format). Adding a new capability here needs a manual move into the right extras group after `uv add` — don't trust the default placement, especially in a project whose entire selling point is "works with zero dependencies."
